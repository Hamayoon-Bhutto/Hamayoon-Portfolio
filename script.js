document.addEventListener('DOMContentLoaded', () => {
  // ---- Hero: nav menu overlay ----
  const heroMenu = document.getElementById('site-menu');
  const heroMenuButton = document.querySelector('.hero-menu-btn');
  const heroMenuClose = document.querySelector('.menu-close');
  let heroMenuPreviousFocus = null;

  if (heroMenu && heroMenuButton && heroMenuClose) {
    heroMenuButton.addEventListener('click', () => {
      heroMenuPreviousFocus = document.activeElement;
      heroMenu.showModal();
      heroMenuButton.setAttribute('aria-expanded', 'true');
      heroMenuClose.focus();
    });
    heroMenuClose.addEventListener('click', () => heroMenu.close());
    heroMenu.addEventListener('close', () => {
      heroMenuButton.setAttribute('aria-expanded', 'false');
      if (heroMenuPreviousFocus) heroMenuPreviousFocus.focus();
    });
    heroMenu.querySelectorAll('a').forEach(link => link.addEventListener('click', (event) => {
      const hash = link.getAttribute('href');
      if (hash && hash.startsWith('#')) {
        event.preventDefault();
        heroMenu.close();
        const target = document.querySelector(hash);
        if (target) window.setTimeout(() => target.scrollIntoView({ behavior: 'smooth' }), 50);
      } else {
        heroMenu.close();
      }
    }));
  ...
  if (!heroReducedMotion.matches && heroRoles.length > 1) {
    ...
    let heroTabVisible = true;
    let heroOnScreen = true;

    const advanceHeroRole = () => {
      heroRoles[heroRoleIndex].classList.remove('is-active');
      heroRoleIndex = (heroRoleIndex + 1) % heroRoles.length;
      window.setTimeout(() => {
        heroRoles[heroRoleIndex].classList.add('is-active');
        if (heroHeading) heroHeading.textContent = heroRoles[heroRoleIndex].textContent;
      }, HERO_EXIT);
      scheduleHeroRole();
    };

    const scheduleHeroRole = () => {
      window.clearTimeout(heroRoleTimer);
      if (heroTabVisible && heroOnScreen) heroRoleTimer = window.setTimeout(advanceHeroRole, HERO_HOLD + HERO_EXIT);
    };

    document.addEventListener('visibilitychange', () => {
      heroTabVisible = !document.hidden;
      scheduleHeroRole();
    });
    ...
    if (heroSectionEl && 'IntersectionObserver' in window) {
      new IntersectionObserver(entries => {
        ...
        scheduleHeroRole();
      }, { threshold: 0.15 }).observe(heroSectionEl);
    }
